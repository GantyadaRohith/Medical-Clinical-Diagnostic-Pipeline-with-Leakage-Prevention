...
<style>
    ...
    /* Glassmorphism Cards */
    .glass-card,
    div[class*="st-key-concept_card"],
    div[class*="st-key-dataset_overview_card"],
    div[class*="st-key-secure_results_card"],
    div[class*="st-key-leaky_results_card"],
    div[class*="st-key-gap_card"],
    div[class*="st-key-roc_card"],
    div[class*="st-key-secure_cm_card"],
    div[class*="st-key-leaky_cm_card"],
    div[class*="st-key-importance_card"],
    div[class*="st-key-input_panel_card"],
    div[class*="st-key-risk_summary_card"],
    div[class*="st-key-trace_card"] {
...
# ---------------------------------------------
with tab_concept:
    with st.container(key="concept_card"):
        ...
        with col_l1:
            st.markdown("""
            <div class="leaky-box">
                <h4>🚫 The Naive / Leaky Preprocessing Flow</h4>
                <strong>What happens:</strong>
                <ol>
                    <li>Imputation values (e.g. mean) or scaling parameters (e.g. min, max, variance) are computed globally across the <strong>entire dataset</strong>.</li>
                    <li>Categorical encoders are fit globally, learning all categorical boundaries including rare levels in the test set.</li>
                    <li>The preprocessed data is then split into training and validation folds.</li>
                </ol>
                <strong>Why it leaks:</strong><br>
        ...
        with col_l2:
            ...
            <div class="secure-box">
                <h4>🛡️ The Secure Pipeline Flow</h4>
                <strong>What happens:</strong>
                <ol>
                    <li>Preprocessing transformations are wrapped into a Scikit-Learn <code>Pipeline</code>.</li>
                    <li>During Cross-Validation, the preprocessing parameters (imputer mean, scaler variance, one-hot categories) are computed <strong>solely from the training split</strong> of that specific fold.</li>
                    <li>The transformations are then applied to the validation split using the parameters fit from the training split.</li>
                </ol>
                <strong>Why it is secure:</strong><br>
                It mathematically guarantees that <strong>no information</strong> from the validation fold is visible during model training, mirroring real-life clinical application.
            </div>
            """, unsafe_allow_html=True)
...
with tab_data:
    if data_info:
        with st.container(key="dataset_overview_card"):
...
with tab_metrics:
    ...
    if 'train_results' in st.session_state:
        ...
        with col_m1:
            with st.container(key="secure_results_card"):
        ...
        with col_m2:
            with st.container(key="leaky_results_card"):
        ...
        with st.container(key="gap_card"):
        ...
        with col_c1:
            with st.container(key="roc_card"):
        ...
        with col_c2:
            with st.container(key="secure_cm_card"):
        ...
        with col_c3:
            with st.container(key="leaky_cm_card"):
        ...
        with st.container(key="importance_card"):
...
# ---------------------------------------------
with tab_predict:
    with st.container(key="input_panel_card"):
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    with col_r1:
                        ...
                        with st.container(key="risk_summary_card"):
